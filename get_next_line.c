...
char	*get_next_line(int fd)
{
	...
	if (!sbuf)
	{
		sbuf = malloc(sizeof(char) * BUFFER_SIZE);
		*sbuf = 0;
		if (!sbuf)
			return (NULL);
	}
	line_len = 0;
	l_line = NULL;
	buf.len = ft_strllen(sbuf, 0, BUFFER_SIZE);
	buf.rlen = ft_strllen(sbuf, '\n', buf.len);
	buf.fd = fd;
	if (!loop(&l_line, &line_len, &sbuf, &buf))
		return (NULL);
	if (buf.len == -1 || l_line == NULL)
		return (free_all(l_line, &sbuf));
	*sbuf = 0;
	if ((size_t)buf.len > buf.rlen)
		save(&sbuf, sbuf + buf.rlen, buf.len - buf.rlen);
	return (get_line(l_line, line_len, &sbuf));
}
